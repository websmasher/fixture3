...
Build `goldencheck`: a reusable CLI for fixture-based golden output checks.
...
The first runnable version of `goldencheck` must check its own fixture suite. Before the CLI can self-host, bootstrap verification may use shell scripts that inspect the repository and run static tools. Those scripts are temporary scaffolding, not tests.
...
After the first runnable command path exists, add a `goldencheck-self` suite that runs the built CLI against local sample fixtures.
