...
Implement the first self-hosting `goldencheck check` slice with manifest-driven verification.
...
1. Expand `.plans/2026-05-13-150929-goldencheck-architecture.md.manifest.toml` with CLI modules, allowed module dependencies, `check` command expectations, generated output paths, and self-hosting command metadata.
...
3. Implement one CLI command: `goldencheck check --suite <name> --manifest <path>`.
4. Add a minimal self-hosting fixture suite in `goldencheck.yaml`.
5. Make `scripts/verify-all.sh` run every verifier layer, including `goldencheck check`.
